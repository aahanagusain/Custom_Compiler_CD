...
class Pipeline:
    ...
    def optimize_only(self, novelang_code):
        """Run lexer → parser → optimizer → codegen to optimize NoveLang code."""
        start = time.time()
        try:
            ...
            optimized_code = codegen.generate(opt_ast)

            # Count original vs optimised lines for stats
            orig_lines = [l for l in novelang_code.strip().split('\n') if l.strip()]
            opt_lines  = [l for l in optimized_code.strip().split('\n') if l.strip()]
            lines_removed = max(0, len(orig_lines) - len(opt_lines))

            elapsed = round((time.time() - start) * 1000, 2)
            return {
                'success': True,
                'optimized_code': optimized_code,
                'original_code': novelang_code,
                'stats': {
                    'original_lines': len(orig_lines),
                    'optimized_lines': len(opt_lines),
                    'lines_removed': lines_removed,
                    'passes': ['Constant Folding', 'Algebraic Simplification',
                               'Dead Code Elimination', 'Boolean Identity'],
                },
                'time_ms': elapsed,
            }
